...
def gaussian_filter_density(img, points):
    ...
    for i, pt in enumerate(points):
        ...
        if int(pt[1]) < img_shape[0] and int(pt[0]) < img_shape[1]:
            ...
            continue
        if gt_count > 1:
            sigma = min((distances[i][1] + distances[i][2] + distances[i][3]) * 0.1, 2.0)  # 限制 sigma 不超过 2.0
        else:
            sigma = min(np.average(np.array(pt2d.shape)) / 2. / 2., 2.0)  # 限制 sigma 不超过 2.0
...
if __name__ == "__main__":
    ...
    root = r'D:\soapberry\mengqw\CCTrans-main2\CCTrans-main\dataset'
