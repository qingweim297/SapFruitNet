...
def cal_new_tensor(img_tensor, min_size=256):
    ...
    if min(h, w) < min_size:
        ...
        if ratio_h >= ratio_w:
            img_tensor = F.interpolate(img_tensor, (min_size, int(min_size / h * w)), mode='bilinear',
                                       align_corners=True)
        else:
            img_tensor = F.interpolate(img_tensor, (int(min_size / w * h), min_size), mode='bilinear',
                                       align_corners=True)
    return img_tensor
...
parser.add_argument('--model-path', type=str, default='best_model_mae-46.64_epoch-172.pth',
                    help='saved model path')
parser.add_argument('--data-path', type=str, default='dataset/part_A_final',
                    help='dataset path')
